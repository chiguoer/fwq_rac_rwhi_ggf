...
class ThetaL1Cost(object):
    ...
    def __init__(self, weight=1.0, pc_range=None, point_cloud_range=None, map_size=None, polar_radius=None, **kwargs):
        self.weight = weight
        # 兼容配置字段：point_cloud_range/pc_range 统一落到 pc_range
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        if pc_range is None:
            raise ValueError(
                "ThetaL1Cost requires pc_range/point_cloud_range in cfg, "
                "e.g. theta_cost=dict(type='ThetaL1Cost', pc_range=point_cloud_range)."
            )
        map_size, polar_radius = compute_map_size_and_radius(
            pc_range, map_size=self.map_size, polar_radius=self.polar_radius
        )
