...
# ============================================================
# 全链路统一的极坐标常量 - 单一来源 (Single Source of Truth)
# 其他模块通过 from .bbox.utils import R_MAX 导入
# ============================================================
...
def theta_d2xy_coods(theta_d_coords, map_size=None, r=None):
    ...
    【注意】map_size 和 r 使用模块常量 MAP_SIZE 和 R_MAX 作为默认值
    全链路必须保持一致，修改需同步 rwhi.py 和 racformer_head.py
    """
    if map_size is None:
        map_size = MAP_SIZE
    if r is None:
...
def xy2theta_d_coods(xy_coords_norm, map_size=None, r=None, norm=True):
    """
    笛卡尔坐标 (x, y) → 极坐标 (θ, d)
