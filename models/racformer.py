...
class RaCFormer(MVXTwoStageDetector):
    ...
    def _use_legacy_baseline_path(self):
        """Compatibility mode for baseline path without RWHI/GGF radar query injection."""
        return bool(
            self.compat_2025
            and not getattr(self.pts_bbox_head, 'use_rwhi', False)
            and not getattr(self.pts_bbox_head, 'ggf_enabled', False)
        )
    ...
    def forward_pts_train(self,
        ...
        Args:
            ...
            radar_points (torch.Tensor, optional): Radar points for RWHI.
            radar_mask (torch.Tensor, optional): Valid mask for radar points.
    ...
    def forward_train(self,
        ...
        if self._use_legacy_baseline_path():
        ...
        # 准备 RWHI 所需的雷达点云 (使用当前帧)
        rwhi_radar_points, rwhi_radar_mask = self._prepare_rwhi_radar_points(radar_points)

        losses = self.forward_pts_train(img_feats, bev_feats, radar_bev_feats, depth, gt_bboxes_3d, gt_labels_3d, gt_depth, img_metas,
                                        radar_points=rwhi_radar_points, radar_mask=rwhi_radar_mask, gt_bboxes_ignore=gt_bboxes_ignore)
        return losses
    ...
    def _prepare_rwhi_radar_points(self, radar_points):
        ...
        Args:
            radar_points: 雷达点云列表 (多帧) 或 tensor
        
        Returns:
            rwhi_radar_points: [B, M, C] 雷达点云 tensor
            rwhi_radar_mask: [B, M] 有效点掩码
        """
        ...
        if isinstance(radar_points, (list, tuple)):
            radar_pts = radar_points[0]  # 当前帧
        else:
            radar_pts = radar_points
        
        # 处理 list of tensors (batch)
        if isinstance(radar_pts, (list, tuple)):
            ...
            for b, pts in enumerate(radar_pts):
                num_pts = pts.shape[0]
                if num_pts > 0:
                    rwhi_radar_points[b, :num_pts] = pts[:, :num_features]
                    rwhi_radar_mask[b, :num_pts] = 1.0
            
            return rwhi_radar_points, rwhi_radar_mask
        else:
            # Already a tensor [B, M, C]
            if radar_pts.dim() == 3:
    ...
    def simple_test_pts(self, x, bev_feats, radar_bev_feats, img_metas, rescale=False, radar_points=None, radar_mask=None):
