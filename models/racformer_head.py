...
class RaCFormer_head(DETRHead):
    ...
    def _is_compat_2025_baseline(self):
        return bool(self.compat_2025 and (not self.use_rwhi) and (not self.ggf_enabled))
    ...
    def _forward_compat_2025(self, mlvl_feats, lss_bev_feats, radar_bev_feats, img_metas):
        """Baseline compatibility path aligned with 2025-09-01 public chain."""
        query_bbox = self.init_query_bbox.weight.clone()  # [Q, 10]
        batch_size = lss_bev_feats.shape[0]
        query_bbox = query_bbox.view(1, self.num_query, 10).repeat(batch_size, 1, 1)

        query_bbox, query_feat, attn_mask, mask_dict = self.prepare_for_dn_input(
            batch_size, query_bbox, None, self.label_enc, img_metas, force_legacy=True
        )
    ...
    def forward(self, mlvl_feats, lss_bev_feats, radar_bev_feats, img_metas, radar_points=None, radar_mask=None):
        ...
        """
        if self._is_compat_2025_baseline():
            return self._forward_compat_2025(mlvl_feats, lss_bev_feats, radar_bev_feats, img_metas)
        ...
        # ============ GGF2.0：获取 GGF 模块并传递给 Transformer ============
        ggf_module = None
    ...
    def prepare_for_dn_input(self, batch_size, init_query_bbox, init_query_feat, label_enc, img_metas, force_legacy=False):
        """
        ...
        Args:
            ...
            init_query_feat: [B, Q, embed_dims] 初始 query 特征 (可为 None)
            label_enc: 标签编码器
            img_metas: 图像元信息
            force_legacy: 是否启用 2025 兼容口径
        """
        ...
        if init_query_feat is None:
            ...
            init_query_feat = torch.cat([init_query_feat, indicator0], dim=1).repeat(batch_size, 1, 1)
        elif init_query_feat.dim() == 2:
            init_query_feat = init_query_feat.unsqueeze(0).repeat(batch_size, 1, 1)
        ...
        if self.training and self.dn_enabled:
            ...
            if force_legacy:
                indicator1 = torch.ones([known_feat_expand.shape[0], 1], device=device)
            else:
                # 【修复】指定 dtype 确保 AMP/FP16 兼容性
