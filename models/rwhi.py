...
class RWHIModule(BaseModule):
    ...
    def _topk_to_anchors(self, S, alpha_map, batch_size, device, topk_k=None):
        ...
        使用「硬前向 + Straight-Through 反向」可微 Top-K：前向与硬 Top-K 一致，反向梯度经 softmax 回传到 S。
        """
        H = W = self.bev_grid_size
        ...
        alpha_flat = alpha_map.view(B, -1)  # [B, H*W]
        alpha_topk_hard = torch.gather(alpha_flat, 1, topk_idx)  # [B, K]
        alpha_topk_hard = alpha_topk_hard.clamp(0.0, 1.0).unsqueeze(-1)  # [B, K, 1]
        alpha_soft = torch.sum(soft_p * alpha_flat, dim=-1, keepdim=True)  # [B, 1]
        alpha_soft = alpha_soft.expand(B, K, 1)  # [B, K, 1]
        alpha_topk = alpha_topk_hard.detach() + (alpha_soft - alpha_soft.detach())
