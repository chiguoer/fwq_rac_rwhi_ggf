...
# 训练排程
# 公平对比口径：ablation 三组统一训练预算
ABLATION_TOTAL_EPOCHS=24
...
declare -a COMPAT_OVERRIDES=(
  "compat_2025_train=True"
  "model.compat_2025=True"
  "model.pts_bbox_head.compat_2025=True"
  "model.pts_bbox_head.transformer.compat_2025=True"
)
...
run_experiment() {
  ...
  local -a merged_overrides=("${COMMON_OVERRIDES[@]}" "${COMPAT_OVERRIDES[@]}" "${exp_overrides[@]}")
...
run_ablation_phase() {
  echo "========== Run fair 3-group ablation =========="
  local -a schedule=(
    "total_epochs=${ABLATION_TOTAL_EPOCHS}"
  ...
  run_experiment "ablation" "rwhi_ggf_on" "${RWHI_GGF_CONFIG}" "${overrides[@]}"
  ...
  # 3) baseline
  run_experiment "ablation" "baseline_racformer" "${BASELINE_CONFIG}" "${schedule[@]}"
}
