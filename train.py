...
def main():
    ...
    compat_2025_train = bool(cfgs.get('compat_2025_train', False))
    ...
    io_main_rank = local_rank if compat_2025_train else rank
    if io_main_rank == 0:
    ...
    logging.info('Train iters per epoch: %d', len(train_loader))
    logging.info('Global batch size: %d', cfgs.batch_size * world_size)
    ...
    sync_bn = True if compat_2025_train else cfgs.get('sync_bn', True)
    ...
    if compat_2025_train and args.max_iters is not None:
        logging.warning('compat_2025_train=True 时忽略 --max_iters，使用 EpochBasedRunner。')
    if compat_2025_train and args.rank_debug_interval:
        logging.warning('compat_2025_train=True 时忽略 --rank_debug_interval。')
